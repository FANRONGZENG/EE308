...
#To change a '.c'or'.cpp' file into '.txt'
def change_type(path):
    import os
    name, type = os.path.splitext(path)
    work_dir, file = os.path.split(path)

    os.rename(  # 实现重命名操作
        os.path.join(work_dir, name + type),
        os.path.join(work_dir, name + '.txt'))
    return name+'.txt'


def main():
    #input the path and command level
    path = input('please enter your path: ')#C:\Users\65660\Desktop\LAB2\lab2.cpp
